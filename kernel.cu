...
void kernel_wrapper(const cv::Mat &mat_ref, const cv::Mat &mat_dst)
{
	...
	for (int timing_idx = 0; timing_idx < itteration_num; ++timing_idx)
	{
		...
		//cudaMemcpyAsync(d_img_ref_float, h_img_ref_float, REAL_SIZE, cudaMemcpyDeviceToHost, stream[1]); //HOST -> DEVICE
		//cudaMemcpyAsync(d_img_dst_float, h_img_dst_float, REAL_SIZE, cudaMemcpyDeviceToHost, stream[1]); //HOST -> DEVICE
		cudaMemcpy(d_img_ref_float, h_img_ref_float, REAL_SIZE, cudaMemcpyHostToDevice); //HOST -> DEVICE
		cudaMemcpy(d_img_dst_float, h_img_dst_float, REAL_SIZE, cudaMemcpyHostToDevice); 
